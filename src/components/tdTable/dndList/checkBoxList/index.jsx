...
const CheckBoxList = (props) => {
  ...
  // 拖拽功能
  const onDrop = ({ dragNode, dragNodesKeys, dropPosition, node }) => {
    const _treeData = treeData;
    // 删除原节点
    dragNodesKeys.forEach((element) => {
      _treeData.map((item, index) =>
        item.key === element ? _treeData.splice(index, 1) : null
      );
    });
    // 插入新节点
  ...
  const onCheck = (val) => {
    const _treeData = treeData;
    _treeData.map(
      (item, index) => (item.show = val.includes(item.key) ? true : false)
    );
    setCheckedKeys(val);
    handelConcatList(type, [..._treeData]);
  };
